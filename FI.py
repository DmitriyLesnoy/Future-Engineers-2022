...
while 1:
    ...
    if state=='main':
        ...
        delta_banka=0
        k=4  # 4 - чем больше k тем меньше отворот по x
        ...
        if area_green is not None or area_red is not None:
            ...
            if time.time()>=timer_banka+0.4 and flag_banka:
        ...
        if (find_wall(HSV_black,500) and delta_banka==0) or (max_r==0 and max_l==0 and delta_banka==0) :
            if direction==1:
                p=25
            else:
                p=-25
