...
def Find_wall(frame_show, flag_draw=True):

    x1, y1 = 320 - 20, 210+delta_wall
    x2, y2 = 320 + 20, 250+delta_wall
    # вырезаем часть изображение
    frame_crop = frame_show[y1:y2, x1:x2]
    # frame_crop_show = frame_show[y1:y2, x1:x2]
    # cv2.imshow("frame_crop", frame_crop)
...
def Find_box(frame_show, flag_draw=True):
    ...
    frame_crop_show = frame_show[y1:y2, x1:x2]
    cv2.rectangle(frame_show, (x1, y1), (x2, y2), (0, 255, 0), 2)
    frame_crop=cv2.GaussianBlur(frame_crop_show,(5,5),cv2.BORDER_DEFAULT)
...
while True:
    ...
    if state == "Manual move":
        ...
        if k == 40:
            robot.move(-speed_manual, 0, 100, wait=False)
        Find_start_line(frame_show,"orange")
        Find_start_line(frame_show,"blue")
    ...
    if state == "Main move":
        ...
        if count_lines >= 12:
            pause_finish = 90 / global_speed-0.5
        ...
        if max_y_left>0 and max_y_right>0:
            if max_y_left+max_y_right>225:
                delta_wall=-20
                side_reg=165
            else:
                delta_wall=0
        ...
        if max_y_right>side_reg:
            max_y_right=side_reg
        if max_y_left>side_reg:
            max_y_left=side_reg

        delta_reg = max_y_right - max_y_left + porog
        p = int(delta_reg * 0.35 + (delta_reg - delta_reg_old) * 0.4)
        delta_reg_old = delta_reg


        if (max_y_right == 0 or flag_doezd_r) and not flag_doezd_l:

            if time.time()<=timer_turn_r+0.1:
                p=-40
            else:
                p=-60
            if povorot:
                p=-60
            if Find_wall(frame_show):
                povorot=True
                global_speed=105
            ...
            if direction==1:
                if max_y_right > 40 and time.time()>timer_turn_r+0:
                    flag_doezd_r = False
                    povorot = False
                    flag_high_line=False
                    global_speed=global_speed_old
        ...
        if (max_y_left==0 or flag_doezd_l) and not flag_doezd_r:
            ...
            flag_doezd_l = True
            if direction==-1:
                if max_y_left > 40 and time.time()>timer_turn_l+0:
                    flag_doezd_l = False
                    povorot = False
                    flag_high_line=False
                    global_speed=global_speed_old
